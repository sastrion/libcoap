...
struct pbuf *
coap_packet_extract_pbuf(coap_packet_t *packet) {
 ...
 */
static void
coap_recv(void *arg, struct udp_pcb *upcb, struct pbuf *p, ip_addr_t *addr,
          u16_t port) {
  coap_endpoint_t *ep = (coap_endpoint_t*) arg;
  coap_packet_t *packet = coap_malloc_type(COAP_PACKET,
...
static coap_tid_t
coap_network_send(struct coap_context_t *context, const coap_endpoint_t *ep,
  ...
  udp_sendto(ep->pcb, pdu->pbuf, &dst->addr, dst->port);
...
coap_new_endpoint(const coap_address_t *addr, int flags) {
  ...
  LWIP_ASSERT("Flags not supported for LWIP endpoints",
              flags == COAP_ENDPOINT_NOSEC);
  ...
  result->cbs.network_send = coap_network_send;
  // FIXME!!!!
  result->cbs.network_read = NULL;
