...
int
coap_option_check_critical(coap_context_t *ctx, coap_pdu_t *pdu,
...
coap_tid_t
coap_send_ack(coap_context_t *context, const coap_endpoint_t *local_interface,
...
coap_retransmit(coap_context_t *context, coap_queue_t *node) {
  ...
  if (context->response_handler) {
    context->response_handler(context, &node->local_if, &node->remote,
                              node->pdu, NULL, node->id);
  }
...
int
coap_handle_message(coap_context_t *ctx, coap_packet_t *packet) {
  ...
  enum result_t {
    RESULT_OK, RESULT_ERR_EARLY, RESULT_ERR
  };
  ...
  error_early: return -result;
}

int
coap_remove_from_queue(coap_queue_t **queue, coap_tid_t id,
                       coap_queue_t **node) {
  coap_queue_t *p, *q;
...
static inline int
token_match(const unsigned char *a, size_t alen, const unsigned char *b,
            size_t blen) {
...
get_wkc_len(coap_context_t *context, coap_opt_t *query_filter) {
  ...
  if (coap_print_wellknown(context, buf, &len, UINT_MAX,
...
coap_wellknown_response(coap_context_t *context, coap_pdu_t *request) {
  ...
  resp = coap_pdu_init(request->hdr->type == COAP_MESSAGE_CON ?
      COAP_MESSAGE_ACK : COAP_MESSAGE_NON,
                       COAP_RESPONSE_CODE(205), request->hdr->id,
                       COAP_MAX_PDU_SIZE);
  if (!resp) {
  ...
  coap_add_option(resp, COAP_OPTION_CONTENT_FORMAT, coap_encode_var_bytes(buf,
  ...
  *resp->data = COAP_PAYLOAD_START;
  ...
  resp->length++;
  len = need_block2 ?
      SZX_TO_BYTES(block.szx) : resp->max_size - resp->length;

  result = coap_print_wellknown(context, resp->data, &len, offset,
                                query_filter);
...
no_response(coap_pdu_t *request, coap_pdu_t *response) {
  ...
  if (nores) {
    val = coap_decode_var_bytes(coap_opt_value(nores),
                                coap_opt_length(nores));
  }
  ...
  if (h) {
    ...
    if (coap_add_token(response, node->pdu->hdr->token_length,
      ...
      if (resource->observable) {
        observe = coap_check_option(node->pdu, COAP_OPTION_OBSERVE,
                                    &opt_iter);
        if (observe) {
...
handle_response(coap_context_t *context, coap_queue_t *sent,
   ...
   * same token.
   */
  coap_cancel_all_messages(context, &rcvd->remote, rcvd->pdu->hdr->token,
                           rcvd->pdu->hdr->token_length);

  /* Call application-specific reponse handler when available. */
  if (context->response_handler) {
    context->response_handler(context, &rcvd->local_if, &rcvd->remote, sent ?
        sent->pdu : NULL,
                              rcvd->pdu, rcvd->id);
  }
}
      ...
      case COAP_MESSAGE_CON: /* check for unknown critical options */
        if (coap_option_check_critical(context, rcvd->pdu, opt_filter) == 0) {
          ...
          if (!response)
            ...
            if (coap_send(context, &rcvd->local_if, &rcvd->remote,
                          response) == COAP_INVALID_TID) {
