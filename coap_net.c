...
coap_new_context(
  ...
  if (!listen_addr) {
	debug("no listen address specified\n");
    return NULL;
  }
  coap_clock_init();
...
coap_send_impl(coap_context_t *context,
  ...
  if(!net_send_to(context->ns, (ipaddr_t *)&dst->addr, dst->port, pdu->mbuf)) {
	  warn("coap_send: sendto\n");
  }
...
get_wkc_len(coap_context_t *context, coap_opt_t *query_filter) {
  ...
  if (print_wellknown(context, NULL, &len, UINT_MAX, query_filter)
